...
void Readme()
{
	printf(" Usage: ./FlannMatching1toN [slow/fast] <source_image_directory> <target_image>\n");
}
	...
	double& matchedPercentage)
{
	...
	for (int srcIndex = 0; srcIndex < static_cast<int>(allSrcDescriptors.size()); srcIndex++)
	{
		...
		matcher.match(targetDescriptors, allSrcDescriptors[srcIndex], oneMatches);

		// Set the imgIdx of oneMatches. Also update the min and max distances.
		for (auto& match: oneMatches)
		{
			match.imgIdx = srcIndex;
	...
	double& matchedPercentage)
{
	...
	matcher.match(targetDescriptors, allMatches);
	...
	// Traverse all the matches and obtain the min and max distances.
	double minDist = DBL_MAX;
	double maxDist = 0;
	for (auto& match: allMatches)
	{
...
int main(int argc, char** argv)
{
    ...
    {
    	surfDetector->detectAndCompute(srcImage, noArray(), oneSrcKeypoints, oneSrcDescriptors);
    ...
    surfDetector->detectAndCompute(targetImage, noArray(), targetKeypoints, targetDescriptors);
	...
	string matchOption(argv[1]);
	if (matchOption == "slow")
	{
		matchedImgIndex = FindBestMatchedImageSlow(allSrcDescriptors, targetDescriptors, allMatches, allGoodMatches, matchedPercentage);
	}
	else if (matchOption == "fast")
	{
		matchedImgIndex = FindBestMatchedImageFast(allSrcDescriptors, targetDescriptors, allMatches, allGoodMatches, matchedPercentage);
	}
	else
	{
