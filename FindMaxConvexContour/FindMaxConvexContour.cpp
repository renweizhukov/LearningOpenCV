...
int main(int argc, char** argv)
{
    ...
    Mat srcImgSharp;
    GaussianBlur(srcImg, srcImgSharp, Size(0, 0), 3);
    addWeighted(srcImg, 1.0, srcImgSharp, -0.5, 0.0, srcImgSharp);
    addWeighted(srcImg, 0.5, srcImgSharp, 1.0, 0.0, srcImgSharp);

    namedWindow("The image after Unsharp Masking", WINDOW_AUTOSIZE);
    imshow("The image after Unsharp Masking", srcImgSharp);

    // Convert the color image after Unsharp Masking into grayscale.
    Mat srcImgSharpGray;
    cvtColor(srcImgSharp, srcImgSharpGray, COLOR_BGR2GRAY);

    // Display the grayscale image after Unsharp Masking.
    namedWindow("The grayscale image after Unsharp Masking", WINDOW_AUTOSIZE);
    imshow("The grayscale image after Unsharp Masking", srcImgSharpGray);

    // Enhance the contrast by doing the histogram equalization.
    Mat srcImgSharpGrayEqualized;
    equalizeHist(srcImgSharpGray, srcImgSharpGrayEqualized);

    // Display the equalized grayscale image.
    namedWindow("The equalized grayscale image after Unsharp Masking", WINDOW_AUTOSIZE);
    imshow("The equalized grayscale image after Unsharp Masking", srcImgSharpGrayEqualized);

    // Compute the mean and standard deviation to get the estimated lower and upper threshold
    // for the Canny edge detection.
    Mat meanMat;
    Mat stdDevMat;
    meanStdDev(srcImgSharpGrayEqualized, meanMat, stdDevMat);
    ...
    Canny(srcImgSharpGrayEqualized, edges, lowerThreshold, upperThreshold);
